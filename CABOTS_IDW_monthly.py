...
#max_depth_bath[max_depth_bath < -1] = np.nan
...
def IDW_bottom_var(
	...
	return(bottom_temp_interp,bottom_mthT_interp,bottom_saln_interp,bottom_mthS_interp)
...
years = np.arange(1980,2023+1).astype(str)
for year in years[:]:
	...
	for month in months[:]:
		...
		start = tt.time()
		inter_temp,inter_mthT,inter_saln,inter_mthS = IDW_bottom_var(
			interpolated_depths,
			max_depth_bath,
			lons_bath,
			lats_bath,
		...
		path_output = '/home/jcoyne/Documents/Bottom_Stats/climatology/monthly/'

		#Set up the .nc file
		nc_out = nc.Dataset(path_output+year+'_'+"%.2d" % month+'.nc','w')

		#File information
		nc_out.title = 'CABOTS Bottom Temperature Salinity Monthly Output' #Temporary title for the .nc file
		...
		nc_out.close()
		print(year+'-'+"%.2d" % month+' done! Time to complete - '+str(np.round((end-start)/60,2))+' minutes')



#Create the monthly climatology files
...
for month in months:
	...
	ds = ds.mean(dim='time')
	ds.to_netcdf(path_output+'bottomclim_'+"%.2d" % month+'.nc')
